  ...
  $conn = mysqli_connect($servername, $username, $pw, $dbname);
         ...
         $fileSize = $file["size"]
           ...
           if(in_array($fileExt, $allow)){
             if($fileError == 0){
               if($fileSizeMB < 2.097152){
                 $imgName = $path;
                 $fileDest = "./uploads/" . $imgName;   //FILE DESTINATION OF IMAGES UPLOADED BY USER

                 $allSQL = "SELECT * FROM Images;";
     ...
     if($stm = $conn->prepare($orderQuery)){
       ...
       if(!mysqli_stmt_prepare($stm, $orderQuery)){
        ...
        //SWITCH FUNCTION for sorting
          switch($order){
            case "PhotoName":
              $orderQuery= "SELECT * FROM Images ORDER BY titleGallery ASC";
            ...
            case "Date":
              $orderQuery= "SELECT * FROM Images ORDER BY dateGallery ASC";
            ...
            case "Location":
              $orderQuery="SELECT * FROM Images ORDER BY locGallery ASC";
